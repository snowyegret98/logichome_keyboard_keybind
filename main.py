...
def check_key(key):
    if key in [Key.up, Key.down, Key.left, Key.right, KeyCode(char=left_mouse), KeyCode(char=right_mouse), KeyCode(char=shift_left_mouse), KeyCode(char=shift_right_mouse)]:
        if key == Key.up:
